...
📋 НОВЫЕ ПРИМЕРЫ - СОГЛАСИЕ/ОТКАЗ НА ПРЕДЛОЖЕНИЯ АССИСТЕНТА:
- Ассистент: "Могу напомнить через неделю?" → Клиент: "Да, конечно" → создать напоминание на 7 дней
- Клиент: "Нет, не пишите" → отменить все активные напоминания
- Ассистент: "А через месяц напишу?" → Клиент: "Хорошо" → создать напоминание на 30 дней
...
📋 ПРИМЕРЫ ИНТЕРПРЕТАЦИИ СОГЛАСИЙ/ОТКАЗОВ НА ПРЕДЛОЖЕНИЯ АССИСТЕНТА:
...
def process_single_reminder(reminder, model, activate_immediately=True, batch_context=None):
    ...
    try:
        ...
        # Формируем промпт безопасным способом
        verify_prompt = PROMPT_VERIFY_REMINDER.replace("{special_rules}", special_rules_text)
        verify_prompt = verify_prompt.replace("{client_context}", context)
        verify_prompt = verify_prompt.replace("{all_active_reminders}", "\n".join(reminders_text) if reminders_text else "Нет других активных напоминаний")
