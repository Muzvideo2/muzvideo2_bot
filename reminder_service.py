...
📋 СПЕЦИАЛЬНЫЕ СЦЕНАРИИ - СОГЛАСИЕ/ОТКАЗ НА ПРЕДЛОЖЕНИЯ БОТА:

✅ СОГЛАСИЕ НА ПРЕДЛОЖЕНИЕ БОТА О НАПОМИНАНИИ:
Контекст: Бот предлагает напоминание в предыдущем сообщении
Клиент соглашается:
- "Да, конечно"
- "Хорошо"
- "Да, давайте"
- "Согласна"
- "Можно"
→ Создай напоминание на время, которое бот предложил в ПРЕДЫДУЩЕМ сообщении

Пример диалога:
Бот: "Я могу Вам мягко напомнить через неделю о нашем разговоре?"
Клиент: "Да, конечно"
...
Контекст: Бот предлагает другой срок напоминания после отказа
Клиент соглашается:
- "Да, хорошо"
- "Через месяц можно"
- "Согласна на месяц"
→ Создай напоминание на новый срок, предложенный ботом

Пример диалога:
Клиент: "Нет, не пишите"
Бот: "А что если я напишу Вам через месяц?"
Клиент: "Да, хорошо"
→ proposed_datetime: через 30 дней в 19:00
→ reminder_context_summary: "Мягко напомнить о разговоре"

🔍 КАК ОПРЕДЕЛИТЬ КОНТЕКСТ ИЗ ПРЕДЫДУЩИХ СООБЩЕНИЙ:
- Ищи в последних 3-5 сообщениях предложения бота со словами: "напомню", "напишу", "свяжусь"
- Извлекай временные интервалы: "через неделю", "через месяц", "завтра", "через 3 дня"
- Если клиент соглашается БЕЗ контекста предложения бота - НЕ создавай напоминание

ВАЖНО: Эти правила работают ТОЛЬКО если в диалоге есть предложение от бота!
...
📋 НОВЫЕ ПРИМЕРЫ - СОГЛАСИЕ/ОТКАЗ НА ПРЕДЛОЖЕНИЯ БОТА:
- Бот: "Могу напомнить через неделю?" → Клиент: "Да, конечно" → создать напоминание на 7 дней
- Клиент: "Нет, не пишите" → отменить все активные напоминания
- Бот: "А через месяц напишу?" → Клиент: "Хорошо" → создать напоминание на 30 дней
...
📋 ПРИМЕРЫ ИНТЕРПРЕТАЦИИ СОГЛАСИЙ/ОТКАЗОВ НА ПРЕДЛОЖЕНИЯ БОТА:
...
✅ Диалог:
Бот: "Хорошо, я поставлю себе напоминание и через неделю мягко напомню о себе"
Клиент: "Хорошо"
   → proposed_datetime: через 7 дней в 19:00
   → target_conv_id: [conv_id клиента]  
   → reminder_context_summary: "Мягко напомнить о разговоре"
...
📋 ПРАВИЛА ДЛЯ СОГЛАСИЙ НА ПРЕДЛОЖЕНИЯ БОТА:
- "через неделю" (из предложения бота) = через 7 дней в 19:00
- "через месяц" (из предложения бота) = через 30 дней в 19:00  
- "через 3 дня" (из предложения бота) = через 3 дня в 19:00
- Если бот предложил конкретное время - используй его
- Если время не указано в предложении бота = сегодня 19:00
...
⚠️ КРИТИЧЕСКИ ВАЖНО - ПРАВИЛА РАБОТЫ С АКТИВНЫМИ НАПОМИНАНИЯМИ:

🚫 НЕ СОЗДАВАЙ НОВЫЕ НАПОМИНАНИЯ ЕСЛИ:
1. ✅ Уже есть активное напоминание на похожую тему/время
2. ✅ Бот в диалоге УЖЕ подтвердил создание напоминания ("поставил напоминание", "напомню")
3. ✅ Клиент просто ПРОДОЛЖАЕТ тему уже созданного напоминания

💡 ПРИЗНАКИ ТОГО, ЧТО НАПОМИНАНИЕ УЖЕ УСТАНОВЛЕНО:
- Бот говорил: "поставил напоминание", "я напомню", "хорошо, напишу"
- В активных напоминаниях есть похожее по времени/теме
- Клиент НЕ просит новое напоминание, а просто добавляет детали

📅 ИСПОЛЬЗУЙ ВРЕМЕННЫЕ МЕТКИ ДЛЯ АНАЛИЗА:
- Если бот недавно подтвердил создание напоминания
- И клиент просто продолжает тему (без новой просьбы)
- То action = "none" с причиной "напоминание уже установлено"

🔍 ПРИМЕРЫ АНАЛИЗА:
❌ НЕ создавай новое:
[14:30] Клиент: напишите завтра
[14:31] Бот: хорошо, напомню
[14:32] Клиент: я буду дома ← ПРОДОЛЖЕНИЕ темы, НЕ новая просьба
...
⚠️ ВАЖНО: Ты работаешь в связке с AI-агентом постановки напоминаний. СОБЛЮДАЙ ТЕ ЖЕ ПРАВИЛА И ПОНИМАЙ ТЕ ЖЕ КОНТЕКСТЫ!

🎯 ПОНИМАНИЕ ТИПОВ НАПОМИНАНИЙ:

1. **ПРЯМЫЕ ПРОСЬБЫ КЛИЕНТА**: "Напомните мне завтра об оплате"
   → Активируй, если клиент не отменял и не выполнил действие

2. **СОГЛАСИЕ НА ПРЕДЛОЖЕНИЕ БОТА**: Бот предложил → клиент согласился
   Пример: Бот: "Могу напомнить через неделю?" → Клиент: "Да"
   → Это ЛЕГИТИМНОЕ напоминание! НЕ отменяй без веских причин

3. **МЯГКИЕ НАПОМИНАНИЯ О РАЗГОВОРЕ**: Созданы по согласию например после "уезжаю в командировку"
   → Активируй, это не навязчивость, а согласованный контакт

4. **НАПОМИНАНИЯ ДЛЯ АДМИНИСТРАТОРА**: По просьбам клиентов связаться с "Сергеем"
   → Активируй, клиент ПРОСИЛ о контакте с администратором

📋 КРИТЕРИИ ДЛЯ ОТМЕНЫ НАПОМИНАНИЯ:

ОТМЕНЯЙ ТОЛЬКО ЕСЛИ:
1. ✅ Клиент ЯВНО отказался: "Нет, не пишите", "Не надо напоминать"
2. ✅ Клиент уже выполнил действие (купил курс, оплатил, записался) ПОСЛЕ создания напоминания
3. ✅ Клиент явно отказался от услуг школы: "Мне это не интересно", "Не хочу обучаться"
4. ✅ Клиент попросил больше не беспокоить его

НЕ ОТМЕНЯЙ ЕСЛИ:
❌ Клиент просто молчит (это нормально для мягких напоминаний)
❌ Клиент занят работой/командировкой (напоминание может быть актуально потом)
❌ Нет явного отказа в последних сообщениях
❌ Напоминание создано по СОГЛАСИЮ клиента на предложение бота

📋 КРИТЕРИИ ДЛЯ ПЕРЕНОСА НАПОМИНАНИЯ:

ПЕРЕНОСИ ЕСЛИ:
1. ✅ Клиент попросил перенести: "Напишите лучше через месяц"
2. ✅ Клиент указал причину временной недоступности с просьбой написать позже
3. ✅ Есть объективные причины для переноса (отпуск, болезнь) И клиент не отказывался

🔍 СПЕЦИАЛЬНЫЕ КОНТЕКСТЫ ДЛЯ ПОНИМАНИЯ:

1. **КОНТЕКСТ "МЯГКОГО НАПОМИНАНИЯ":**
   Если reminder_context_summary содержит "мягко напомнить о разговоре" - это значит:
   - Клиент согласился на контакт после перерыва
   - Бот предложил, клиент согласился
   - НЕ ОТМЕНЯЙ без явного отказа клиента

2. **КОНТЕКСТ ПРОСЬБЫ СВЯЗАТЬСЯ С АДМИНИСТРАТОРОМ:**
   Если напоминание для администратора содержит "клиент просит связаться" - это значит:
   - Клиент САМ попросил контакта с "Сергеем"
   - Это НЕ навязывание, а выполнение просьбы клиента
   - АКТИВИРУЙ, даже если клиент потом молчит

3. **КОНТЕКСТ НЕДАВНИХ ПОКУПОК:**
   Если клиент купил что-то за последние 24 часа:
   - Возможно, напоминание об оплате уже неактуально
   - Проверь, относится ли покупка к теме напоминания
...
1. Определи ТИП напоминания (прямая просьба / согласие на предложение / просьба связаться с админом)
2. Найди в контексте клиента ЯВНЫЕ отказы или отмены после создания напоминания  
3. Проверь, выполнил ли клиент действие ПОСЛЕ создания напоминания
4. Если сомневаешься - лучше АКТИВИРУЙ (следуй принципу "клиент согласился")

⚠️ КРИТИЧЕСКИ ВАЖНО - ПОНИМАНИЕ ХРОНОЛОГИИ:

🕐 ПРАВИЛО ВРЕМЕННОЙ ПОСЛЕДОВАТЕЛЬНОСТИ:
- Учитывай ТОЛЬКО события ПОСЛЕ времени создания напоминания
- Если клиент отказывался ДО создания текущего напоминания - это НЕ ПРИЧИНА для отмены
- Ключевое понимание: каждое новое напоминание = новый договор с клиентом

📋 СЦЕНАРИЙ "ПЕРЕПОСТАНОВКИ ПОСЛЕ ОТКАЗА":
Пример:
1. Бот: "Напомню через месяц" 
2. Клиент: "Нет, не пишите" (отказ от первого предложения)
3. Бот: "А через 3 месяца?"
4. Клиент: "Да, ок" (согласие на альтернативу)
→ Создается НОВОЕ напоминание на 3 месяца

✅ КАК АНАЛИЗИРОВАТЬ:
- Время создания напоминания: когда клиент сказал "Да, ок"
- Отказ "Нет, не пишите" был ДО создания → НЕ учитывай
- Согласие "Да, ок" привело к созданию → это актуальный договор
- АКТИВИРУЙ напоминание, несмотря на предыдущий отказ

🔍 ПРАКТИЧЕСКИЙ АЛГОРИТМ:
1. Найди время создания напоминания
2. Анализируй ТОЛЬКО сообщения клиента ПОСЛЕ этого времени
3. Игнорируй отказы ДО создания напоминания
4. Ищи новые отказы или отмены ПОСЛЕ создания

📅 КАК РАБОТАТЬ С ВРЕМЕННЫМИ МЕТКАМИ:
- История диалога содержит метки времени в формате [YYYY-MM-DD HH:MM:SS]
- Время создания напоминания указано выше в разделе "ИНФОРМАЦИЯ О НАПОМИНАНИИ"
- Сравнивай время сообщений с временем создания напоминания
- Учитывай ТОЛЬКО сообщения клиента ПОСЛЕ времени создания

💡 ПРИМЕР АНАЛИЗА ХРОНОЛОГИИ:
Время создания напоминания: 2024-01-15 14:33:15
История диалога:
[2024-01-15 14:30:00] Бот: Давайте я Вам напомню через месяц
[2024-01-15 14:31:00] Клиент: Нет, не пишите ← ДО создания (игнорируй)
[2024-01-15 14:32:00] Бот: А что если через 3 месяца?
[2024-01-15 14:33:00] Клиент: Да, ок ← привело к созданию напоминания
[2024-01-15 14:35:00] Клиент: Спасибо ← ПОСЛЕ создания (учитывай)

→ РЕШЕНИЕ: АКТИВИРУЙ (отказ был до создания, согласие привело к созданию)

ВЕРНИ ОТВЕТ СТРОГО В ФОРМАТЕ JSON:
{{
  "should_activate": true/false,
  "reason": "Подробное объяснение решения с указанием типа напоминания и найденных в контексте фактов",
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            if messages:
                ...
                for msg in reversed(messages):
                    ...
                    # КРИТИЧЕСКИ ВАЖНО: Добавляем временные метки для понимания хронологии
                    timestamp = msg['created_at'].strftime('%Y-%m-%d %H:%M:%S') if msg['created_at'] else 'неизвестно'
